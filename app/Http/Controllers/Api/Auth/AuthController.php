...
class AuthController extends Controller
{
    ...
    {
        return $this->success(
                $this->userService->createUser($request->validated()), 'تم تسجيل المستخدم بنجاح',
                201);
    }
    ...
    public function login(LoginRequest $request)
    {
        ...
        if (empty($user) || !Hash::check($request->validated()['password'], $user->password)) {
            return $this->error('الايميل أو كلمة المرور غير صحيحة أو المستخدم غير مسجل', 401); 
        }
        $token = $user->createToken('auth_token', ['*'], now()->addDay())->plainTextToken;

        return $this->success(['token' => $token],'تم تسجيل الدخول بنجاح');
    }
